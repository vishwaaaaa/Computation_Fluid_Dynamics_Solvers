...
for ibfgrp = 1:nbfgrp
  ...
  l{ibfgrp}=A; %stores the node ids of all the nodes in the boundary face
end
...
for i=1:n_edge_inc
    for j=1:nbfgrp
        if any(l{j}==edge_info(i,1)) && any(l{j}==edge_info(i,2))    %checks if for any edge both nodes are listed in the list of nodes of a boundary face 
            edge_info(i,5) = -1*j;
        end
    end
end
